...
export default function FrameworkThreeEs() {
  ...
  return (
    <section id="framework" className="py-24 md:py-32 bg-[var(--bg-primary)] border-t border-[var(--border-subtle)] relative">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20">
          ...
          <Reveal delay={0.1}>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-heading font-bold text-[var(--text-primary)] tracking-tight mb-6">
              The Three E's of <span className="font-heading italic text-[var(--accent-cobalt)]">Enterprise Freedom</span>
            </h2>
          </Reveal>
          ...
          {pillars.map((pillar) => {
            ...
            return (
              <StaggerItem key={pillar.number}>
                <div className="h-full p-8 md:p-10 rounded-sm bg-[var(--bg-card)] border border-[var(--border-subtle)] hover:border-[var(--border-cobalt)] transition-all duration-300 flex flex-col justify-between group relative overflow-hidden">
                  
                  {/* Subtle top indicator */}
                  <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-[var(--accent-cobalt)] to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />

                  <div>
                    {/* Pillar Number & Icon */}
                    <div className="flex items-center justify-between mb-8">
                      <div className="w-14 h-14 rounded-sm bg-[#161619] border border-[var(--border-subtle)] flex items-center justify-center text-[var(--accent-cobalt)] group-hover:border-[var(--border-cobalt)] group-hover:shadow-[0_0_20px_rgba(197,160,89,0.2)] transition-all">
                        <Icon size={26} />
                      </div>
                      <span className="font-heading font-bold text-2xl text-[var(--text-secondary)] group-hover:text-[var(--accent-cobalt)] transition-colors">
                        {pillar.number}
                      </span>
                    </div>

                    {/* Titles */}
                    <h3 className="font-heading font-bold text-2xl text-[var(--text-primary)] tracking-wide mb-1">
                      {pillar.title}
                    </h3>
                    <p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--accent-cobalt)] mb-6">
                      {pillar.subtitle}
                    </p>

                    {/* Description */}
                    <p className="text-sm md:text-[15px] text-[var(--text-secondary)] leading-relaxed font-sans mb-8">
                      {pillar.description}
                    </p>
                  </div>
                  ...
                  <div className="pt-6 border-t border-[var(--border-subtle)] space-y-2.5">
                    {pillar.deliverables.map((item, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs text-[var(--text-primary)]">
                        <span className="w-1.5 h-1.5 rounded-full bg-[var(--accent-cobalt)]" />
                        <span>{item}</span>
                      </div>
                    ))}
                  </div>
